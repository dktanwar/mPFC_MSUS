---
title: "mPFC: Checking for duplicates with dupRadar"
author: "Deepak Tanwar"
date: "<b>Created on:</b> 2020-10-07 <br> <b>Updated on:</b> `r Sys.Date()`"
output:
...
```{r dupRadar-analysis-1, echo=FALSE, cache=FALSE}
library(knitr)
library(rmdformats)
...
# Analyze duplicates
```{r dupRadar-analysis-4 }
dm <- NULL
if (!file.exists("./output/analyzedDuplicates.rds")) {
  dm <- lapply(bamFiles, function(x) {
    a <- analyzeDuprates(
      bam = x,
      gtf = "./input/gencode.vM18.chr_patch_hapl_scaff.annotation.gff3",
...
```{r dupRadar-analysis-5, results='asis', fig.align='center', fig.width=7, fig.height=7}
...
```{r dupRadar-analysis-6, results='asis', fig.align='center', fig.width=8, fig.height=7}
for (i in seq_along(dm)) {
  name <- names(dm)[i]
  cat("\n \n")
  cat(paste("###", name))
...
```{r dupRadar-analysis-7, results='asis', fig.align='center', fig.width=8, fig.height=7}
for (i in seq_along(dm)) {
  name <- names(dm)[i]
  cat("\n \n")
  cat(paste("###", name))
...
```{r dupRadar-analysis-8, results='asis', fig.align='center', fig.width=7, fig.height=7}
for (i in seq_along(dm)) {
  name <- names(dm)[i]
  cat("\n \n")
  cat(paste("###", name))
...
```{r dupRadar-analysis-9, results='asis', fig.align='center', fig.width=7, fig.height=7}
for (i in seq_along(dm)) {
  name <- names(dm)[i]
  cat("\n \n")
  cat(paste("###", name))
...
```{r dupRadar-analysis-11, fig.align='center', fig.width=10, fig.height=7, results='asis'}
for (i in seq_along(dm)) {
  name <- names(dm)[i]
  cat("\n \n")
  cat(paste("###", name))
...
# References
```{r dupRadar-analysis-12}
report::cite_packages(sessionInfo())
```

# SessionInfo
```{r dupRadar-analysis-13 }
